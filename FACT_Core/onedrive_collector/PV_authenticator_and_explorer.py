...
.. note::
    'TITLE'             : OneDrive - Personal Vault in AF-Forensics\n
    'AUTHOR'            : Jihyeok Yang\n
    'TEAM'              : DFRC\n
    'VERSION'           : 0.0.4\n
    ...
    def __login(self):
        ...
        try:
            with sync_playwright() as playwright:
                ...
                if self.__set_auth_value() == FC_ERROR:
                    PRINT('Set Authentication Value Error')
                    return FC_ERROR

                if self.__set_header_value() == FC_ERROR:
                    PRINT('Set Header Value Error')

                if self.__set_normal_file_list() == FC_ERROR:
                    PRINT('Set Normal File List Error')
                    return FC_ERROR

                if self.__set_recycle_file_list() == FC_ERROR:
                    PRINT('Set RecycleBin File List Error')
                    return FC_ERROR

                if self.__set_shared_file_list() == FC_ERROR:
                    PRINT('Set Shared File List Error')
                    return FC_ERROR

                if self.__combine_file_list() == FC_ERROR:
                    PRINT('Combine File List Error')
                    return FC_ERROR

                if self.__devide_file_list() == FC_ERROR:
                    PRINT('Divie File List Error')
                    return FC_ERROR

                if self.__download_thumbnails() == FC_ERROR:
                    PRINT('Collecting Thumbnails Error')
                    return FC_ERROR

                if self.__set_version_history() == FC_ERROR:
                    return FC_ERROR
