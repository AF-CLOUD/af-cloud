    ...
    public class ListAppData
    {
        ...
        public static IList<Google.Apis.Drive.v3.Data.File> DriveListAppData(DriveService service ,UserCredential credential)
        {
            try
            {
                ...
                {
                    Console.WriteLine("{0} {1} {2} {3} {4}", i, file.Name, file.MimeType, file.Shared, file.Trashed);
                    ...
                    if (file.ThumbnailLink != null)
                    {
                        ...
                        string f_thum = file.ThumbnailLink.Replace("220", "1920");

                        string t_name = Path.GetFileNameWithoutExtension(file.Name);
                        using (var mywebClient = new WebClient())
                        {
                            bool y = f_thum.Contains("lh");

                            if (y == false)
                            {
                                string a = f_thum + "&access_token=" + credential.Token.AccessToken;
